...
export default {
  ...
  data () {
    var validateTel = (rule, value, callback) => { // 验证电话号码正则表达式
      var regExp = /^1[0-9]{10}$/
    ...
    return {
      ...
      rules: {
        ...
        phone: [{ required: true, message: '请输入手机号码', trigger: 'blur' }, { validator: validateTel, trigger: 'blur' }],
  ...
  methods: {
    ...
    handleJurisdic (row) {
      window.open(`${window.location.origin}/permission?userId=${row.userId}&name=${row.name}&userName=${row.userName}`, '_blank')
    },
    handleDel (row) {
    ...
    confirmRegale () {
      ...
      self.ajax.post('/api/user/del', {
        ...
        if (res.success) {
          ...
          setTimeout(function () { self.dialog.showDialog = false }, 3000)
        }
      })
    },
